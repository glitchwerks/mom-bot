...
| `dev-app-insights-conn-string` | No — separate AI instances per environment | Azure Application Insights connection string for local dev (provisioned in PR #239, closes #182). **As of issue #199 the container reads `APPLICATIONINSIGHTS_CONNECTION_STRING` from the Container App env (injected via `secretRef` in `infra/modules/containerapp.bicep:191`) rather than reading this KV secret directly at runtime.  The KV secret is retained as the source of that env-var injection and should not be deleted.** | Runtime | Azure portal — @cbeaulieu-gt | On workspace recreation |
| `prod-app-insights-conn-string` | No — separate AI instances per environment | Azure Application Insights connection string for prod (provisioned in PR #239, closes #182). **As of issue #199 the container reads `APPLICATIONINSIGHTS_CONNECTION_STRING` from the Container App env (injected via `secretRef` in `infra/modules/containerapp.bicep:191`) rather than reading this KV secret directly at runtime.  The KV secret is retained as the source of that env-var injection and should not be deleted.** | Runtime | Azure portal — @cbeaulieu-gt | On workspace recreation |
