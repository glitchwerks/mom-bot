...
v1.0.0 is structurally identical to the v1.0.0-rc.0 release candidate apart from the single-line `pyproject.toml` version bump from `1.0.0-rc.0` to `1.0.0`. There is no behavioral delta between the `:main` image at tag time, the `:v1.0.0-rc.0` RC image, and `:v1.0.0`.
